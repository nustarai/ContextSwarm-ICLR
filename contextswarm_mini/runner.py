...
class _ElasticTaskState:
    ...
    candidate_verdicts: dict[str, Verdict] = field(default_factory=dict)
    candidate_order: dict[str, int] = field(default_factory=dict)
...
def _verdict_priority(verdict: Verdict | None) -> tuple[int, float]:
    ...
    status_rank = {
        ...
        "VERIFY_FAIL": 1,
        "CHEATING": -2,
        # A candidate completed before the horizon but still awaiting optional
        # diagnostics supersedes known non-proofs, never a proved snapshot.
        "UNEVALUATED_CANDIDATE": 3,
...
def _select_candidate_sha(
    verdicts: Mapping[str, Verdict],
    completion_order: Mapping[str, int],
) -> str:
    """Select deterministically by diagnostic rank, then solver completion."""
    ...
    return max(
        verdicts,
        key=lambda sha: (
            _verdict_priority(verdicts[sha]),
            int(completion_order.get(sha, -1)),
            sha,
        ),
    )
...
def _run_elastic_cps(
    ...
    *,
    ...
    for task in tasks:
        ...
        baseline_snapshot = broker.capture_handoff_candidate(
            task,
            best_path,
            trusted_root=best_dir,
        )
        baseline_verdict = Verdict(
            ...
            0.0,
            0.0,
            {"candidate_sha256": baseline_snapshot.sha256},
        )
        state.selected_candidate_sha = baseline_snapshot.sha256
        state.candidate_verdicts[baseline_snapshot.sha256] = baseline_verdict
        state.candidate_order[baseline_snapshot.sha256] = 0
        state.best_verdict = baseline_verdict
    ...
    def refresh_selected_candidate(state: _ElasticTaskState) -> None:
        selected_sha = _select_candidate_sha(
            state.candidate_verdicts,
            state.candidate_order,
        )
        # Always project the immutable broker snapshot. The runner must not
        # trust mutable best/result.lean bytes even when the selected SHA did
        # not change (for example after a blocked or buggy worker write).
        ...
        state.selected_candidate_sha = selected_sha
        state.best_verdict = state.candidate_verdicts[selected_sha]
    ...
    def preserve_unevaluated_candidate(
        ...
        state = states[assignment.task_id]
        try:
            snapshot = broker.capture_handoff_candidate(
                state.task,
                candidate,
        ...
        with state.lock:
            ...
            prior = state.candidate_verdicts.get(digest)
            if prior is None or prior.status in {
                "BASELINE_CANDIDATE",
                "UNEVALUATED_CANDIDATE",
            }:
                state.candidate_verdicts[digest] = fallback
            state.candidate_order[digest] = state.candidate_sequence
            try:
                refresh_selected_candidate(state)
            except OSError:
                pass
            preserved = state.selected_candidate_sha == digest
        logger.event(
            "candidate_preserved",
            task_id=assignment.task_id,
            agent_id=assignment.agent_id,
    ...
    # No solver process remains past this point. Re-project every selected
    # immutable snapshot once more before the closeout freezer reads it.
...
def _evaluate_candidate(
    ...
    *,
    ...
    return broker.evaluate_handoff(
        task,
        candidate,
        trusted_root=candidate.parent,
