...
For all sessions, independent proof construction does not ban Lean tactics, known
Mathlib APIs, or bounded `find`/`exact`/`apply` searches. Do not inspect unrelated
files, host paths, other workers, or external completed proofs. If an environment
search becomes expensive, stop it and leave the strongest candidate so it does not
monopolize Judge capacity.
