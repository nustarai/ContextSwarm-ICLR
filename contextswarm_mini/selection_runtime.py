...
class SelectionRuntime:
    ...
    def _trace_stats_impl(
        ...
        with self.selection_store._db() as db:  # package-private read-only projection
            db.execute("BEGIN")
        ...
        for row in rows:
            ...
            candidates.append(
                _RuntimeTraceCandidate(
                    ...
                    token_count=token_count({"title": title, "body": body, "tags": tags}),
    ...
    def _search_impl(
        ...
        *,
        ...
        prior = (
            self.selection_store.get_search_by_request_key(str(request_key))
        ...
        if prior is None:
            prior = self.selection_store.get_search_by_request_key(str(request_key))
    ...
    def broker_search(self, claim: Any, query: str, limit: int) -> Mapping[str, Any]:
        ...
        return self.search(task_id=task_id, actor_id=actor_id, query=query, limit=limit)
