...
This worktree also contains a separate FLT strict-from-scratch smoke task.
`fermat_last_theorem` and `configs/flt_1h_smoke.toml` are experimental only and
do not enter the six-dataset paper matrix. The Agent-visible bundle exposes
only Mathlib and the theorem statement; the dedicated `formal_flt` Judge is
generated by `ContextSwarmJudge/scripts/setup_formal_flt_env.py` and must not
mount the completed FLT source or `.olean` cache into the Agent workspace.
The completed one-hour run and its independent frozen-candidate recheck are
documented in `benchmarks/fermat_last_theorem/EXPERIMENT_REPORT.md`.
