# Verina Formal Task: `verina_advanced_61`
...
- Baseline file: `baseline/verina_advanced_61.lean`.
- Benchmark source: `Goedel-Code-Prover/benchmark/verina_bench.json`
- Original problem id: `verina/verina_advanced_61.lean`
- Subset: `advanced`
- Namespace: global (the upstream task does not declare a namespace)
- Target theorem: `productExceptSelf_spec_satisfied`

## Contract Scope

This task proves only the checked Lean functional contract for
product-except-self. It does **not** certify the O(n) claim in the upstream
natural-language description: both prefix/suffix folds append with `acc ++ [x]`,
so the checked implementation has quadratic list-building cost in Lean's list
model. Runtime-complexity certification is explicitly excluded.
